...
export class AdminShellPage {
  ...
  private topNavCandidates(label: string, href: string): Locator[] {
    return [
  ...
  async gotoDashboard() {
    ...
    await clickFirstVisible(this.topNavCandidates('儀錶板', '/admin/dashboard'), { name: 'dashboard nav' });
    await expect(this.page).toHaveURL(/\/admin\/dashboard/);
  }

  async gotoChildList() {
    await maybeDismissOpenPopover(this.page);
    await clickFirstVisible(this.topNavCandidates('孩童列表', '/admin/child-list'), { name: 'child list nav' });
    await expect(this.page).toHaveURL(/\/admin\/child-list/);
  }

  async gotoQuestionManage() {
    await maybeDismissOpenPopover(this.page);
    await clickFirstVisible(this.topNavCandidates('題目管理', '/admin/question'), { name: 'question nav' });
    await expect(this.page).toHaveURL(/\/admin\/question/);
  }

  async gotoInviteManage() {
    await maybeDismissOpenPopover(this.page);
    await clickFirstVisible(this.topNavCandidates('邀請管理', '/admin/invite'), { name: 'invite nav' });
    await expect(this.page).toHaveURL(/\/admin\/invite/);
  }

  async gotoAbout() {
    await maybeDismissOpenPopover(this.page);
    await clickFirstVisible(this.topNavCandidates('關於我們', '/admin/about'), { name: 'about nav' });
